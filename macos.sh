...
if ! declare -F requirers >/dev/null; then
    source lib/requirers.sh
fi

bot "I'm going to configure macOS."

running "Close any open System Preferences panes, to prevent them from overriding settings we're about to change"
osascript -e 'tell application "System Preferences" to quit';ok

running "Ask for the administrator password upfront"
sudo -v;ok
...
# running "Menu bar: hide the Time Machine, Volume, Battery, Clock, and User icons (using iStat Menus instead)"
...
# 	"/System/Library/CoreServices/Menu Extras/AirPort.menu";ok
...
running "Save screenshots to a folder"
...
running "Expand the following File Info panes: General, Open with, and Sharing"
...
defaults write com.tapbots.TweetbotMac OpenURLsDirectly -bool true;ok
